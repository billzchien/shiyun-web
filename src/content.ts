...
export const learnSections: {
  ...
  },
  {
    ...
    en: [
      ...
      { fig: 'daymaster' },
      { p: 'Reading a person through these four pillars was refined and settled in the Tang and Song dynasties, and every almanac tradition since has been built on it.' },
    ],
    cn: [
      { sub: '四柱' },
      { p: '前面说过，干支最早是用来纪日的。' },
      ...
      { p: '不过，四柱之间并非各自独立。月柱地支由节气锁定：正月固定为寅月，二月固定为卯月。时柱地支由钟点锁定：夜里十一点起即为子时。月柱天干，由年柱天干推出；时柱天干，由日柱天干推出。古人给这两套推法起了名字：正月建寅，寅属虎，推月干的口诀叫「五虎遁」；时辰起始于子，子属鼠，推时干的口诀叫「五鼠遁」。所以八字的八个字里，真正独立的信息只有四项：年柱、月支、日柱、时支，月干与时干都是推出来的。' },
      { p: '既然如此，为什么还要把推出来的天干一并写明？因为四柱之间的生克比对，全部以干支为统一单位开展。日主本身是一个天干，只有把月干、时干补齐，五行生克才有对象，命局格局才能成形。' },
      { sub: '日主' },
      { p: '这八个字里，日柱的天干直接代表你自己，被称为「日主」。你的日主是十天干中的某一个，自然就带着这一干对应的五行与阴阳。剩下的七个字，围绕日主排布，各自与你的日主发生生克，构成属于你的格局。' },
      { fig: 'daymaster' },
      { p: '以日主为核心解读命盘的方法，在唐宋之间逐步定型。之后的上千年里，黄历择日、命理推演，全部都建立在这套四柱体系的基础之上。' },
    ],
  },
  {
    id: 'calendar',
  ...
  },
  {
    ...
    cn: [
      { p: '翻开任意一本老黄历，每天都印着两行字：宜什么，忌什么。' },
      { p: '这是黄历最深入人心的传统。旧时家家户户案头一本通书，动土、嫁娶、出行、开市，先翻宜忌再定日子，择日而行，是绵延千年的生活习惯。' },
      { p: '宜忌从哪来？历代形成了不止一套推法。《史记》里记过一桩事：汉武帝召集各派占家，问某日可否娶亲，七家给出七个答案，从大吉到大凶都有，吵到最后不了了之。到了清代，朝廷官修《协纪辨方书》，把历代神煞做了一次大清理，此后各家黄历大都以它为本。' },
      { p: '但统一底本之下，神煞取哪些、吉凶怎么权衡，各家至今仍各有做法，所以不同黄历对同一天的宜忌，常常并不一致。宜忌从来没有唯一的标准答案，它是一套参照，不是一纸判决。' },
      { p: '时运的宜忌，同样承自《协纪辨方书》一系的传统条目，再多走一步：先按当日筛出传统宜忌，再对照你的命盘取舍。所以你看到的宜忌，和别人看到的不同，和别家黄历印的也不同，它是你的那一份。读到这里，你已经认识了黄历全部的零件。' },
    ],
  },
];
